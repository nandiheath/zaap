...
- **Tooling:** Helm and Kustomize are pinned through Hermit. Repository work now uses milestone-first, path-owned planning contracts under `tasks/`.
...
Future outcome planning is under `tasks/milestones/planned/`. Spawn path-owned child tasks under `tasks/planned/` only after reviewing current `main`, dependencies, ownership, and rollout order.

## Recent changes

- **2026-08-02 — Milestone-first planning:** added repository guidance plus task and milestone templates, retained production readiness as a future outcome reminder without prematurely claiming implementation, and passed `agent-workspace repo-tasks validate --root .`.
